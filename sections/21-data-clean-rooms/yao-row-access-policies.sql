...
use role alice;
create table alice_db.public.my_wealth as
  select 1110000 as wealth;

-- Bob hides his wealth in his table
use role bob;
create table bob_db.public.my_wealth as
  select 1250000 as wealth;

-- Bob grants usage on his protected table to Alice
create table bob_db.public.allowed_statements(statement varchar);
...
create row access policy bob_db.public.mask_wealth
  ...
  current_role() in ('ACCOUNTADMIN', 'BOB')
    or exists (
      select statement
      from  bob_db.public.allowed_statements
...
-- Bob allows Alice to run only a specific query
insert into bob_db.public.allowed_statements (statement)
values ('select case
